...
void setup() {
  ...
  digitalWrite(FrequencyScalerPin1,HIGH);
  digitalWrite(FrequencyScalerPin2,LOW);
...
void loop() {
  ...
  while(1){
    _quarterTurn++;
    STEER((2 * _clockwise - 1),100);
    while(GYRO_GET_YAW() + yawCorrection < (_quarterTurn * 90 * (2 * _clockwise - 1))%360);
    STEER((1 - 2 * _clockwise),100);
    if(_quarterTurn == 12) break;
    while(ORANGE_BLUE_DETECTOR() == 2){
      if(RIGHT_ULTRASONIC() > (13 - _clockwise * 6)){
        STEER(1,100);
        STEER(-1,200);
        STEER(1,100);
      }else if(LEFT_ULTRASONIC() > (7 + _clockwise * 6))
        STEER(-1,100);
        STEER(1,200);
        STEER(-1,100);
    }
